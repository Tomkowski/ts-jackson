...
var testPlaylistJson = {
    ...
    description: 'Throwing a soft party, having drinks or just chilling with your friends.',
    images: [
        {
            height: 640,
...
describe('Playlist Entity', function () {
    test('deserialize', function () {
        var playlist = Playlist_1.default.deserialize(testPlaylistJson);
        expect(playlist).toMatchObject({
            id: testPlaylistJson.id,
            name: testPlaylistJson.name,
            description: testPlaylistJson.description,
            icon: Image_1.default.deserialize(testPlaylistJson.images[2]),
            backgroundImage: Image_1.default.deserialize(testPlaylistJson.images[0]),
            tracks: testPlaylistJson.tracks.items.map(function (trackJson) {
                return Track_1.default.deserialize(trackJson);
            }),
        });
        expect(playlist instanceof Playlist_1.default).toBeTruthy();
        expect(playlist.icon instanceof Image_1.default).toBeTruthy();
        expect(playlist.backgroundImage instanceof Image_1.default).toBeTruthy();
    });
    test('serialize', function () {
        var playlist = Playlist_1.default.deserialize(testPlaylistJson);
        expect((0, index_1.serialize)(playlist)).toEqual({
            id: playlist.id,
            name: playlist.name,
            description: playlist.description,
            images: [
                (0, index_1.serialize)(playlist.backgroundImage),
                undefined,
                (0, index_1.serialize)(playlist.icon),
            ],
            tracks: {
                items: playlist.tracks.map(function (track) { return (0, index_1.serialize)(track); }),
            },
        });
    });
});
